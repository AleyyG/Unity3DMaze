...
public class MazeRendererLast : MonoBehaviour
{
    public static MazeRendererLast Current;
    ...
    void Update()
    {
        ...
        {
            float distance = Vector3.Distance(hole.transform.position, BallMovementLast.Current.transform.position);
            levelBar.minValue = -levelBar.maxValue;
            levelBar.value = levelBar.maxValue - distance;
            fill.color = gradient.Evaluate(levelBar.value);
        }
        if (BallMovementLast.Current.isFinished)
